...
(function(global, undefined) {
    ...
    function require() {
        ...
        for (var i = 0, len = ids.length; i < len; i++) {
            ...
            moduleUrl    = normalizeObj.url + '.js?v='+requireConf.site_version;
    ...
    function define() {
        ...
        if (!id) {
            ...
            for (var key in idUrlMap) {
                ...
                // else {
                //     if (idUrlMap[key].indexOf('..') !== -1) {
                //         var terms = idUrlMap[key].split('/');
                //         for (var i = 0, len = terms.length; i < len; i++) {
                //             if (terms[i] === '..') {
                //                 terms.splice(i - 1, 2);
                //             }
                //         }
                //         if (terms.join('/') === curScript) {
                //             id = key;
                //             break;
                //         }
                //     }
    ...
    function handleRealDeps(realDep, requireId, curModule) {
        ...
        if (REG.PREFIX_RELATIVE.test(realDep)) {
            ...
            realDepUrl = normalizeObj.url + '.js?v='+requireConf.site_version;
        }
